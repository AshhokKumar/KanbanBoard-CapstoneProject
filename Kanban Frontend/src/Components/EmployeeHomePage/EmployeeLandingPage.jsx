...
import EmployeeNavbar from '../EmployeeNavbar/EmployeeNavbar.jsx';
...
const taskUrl = `${AUTH_API_BASE_URL}/api/v1/my-tasks`;
...
const LandingPage = () => {
  ...
  useEffect(() => {
    ...
    console.log("Token generated:",token);
        ...
        const tasksByStatus = { todo: [], research: [], inprogress: [], submittedforreview: [] };
    ...
    setFilteredTasks(() => {
      return Object.keys(tasks).reduce((acc, status) => {
        acc[status] = tasks[status].filter(
          (task) =>
            searchText === "" || task.title.toLowerCase().includes(searchText.toLowerCase()) // Filter by title
        );
        return acc;
      }, {});
    });
...
useEffect(() => {
  setFilteredTasks(() => {
    return Object.keys(tasks).reduce((acc, status) => {
      acc[status] = tasks[status].filter(
        (task) => filter === "All" || task.priority.toLowerCase() === filter.toLowerCase()
      );
      return acc;
    }, {});
  });
  ...
  return (
    <>
    <EmployeeNavbar  filter={filter} setFilter={setFilter} searchText={searchText} setSearchText={setSearchText}/>
                        ...
                        >
                          ...
                          <AvatarGroup sx={{bgcolor: getPriorityColor(task.priority)}} max={4}>
                           ...
                           <Box display="flex" justifyContent="space-between" mt={1}>
                              <IconButton onClick={() => navigate(`/employeeUpdateTask/${task.id}`)} sx={{ 
    color: "white", 
