...
const UpdateTask = () => {
  ...
  const isAdmin =localStorage.getItem("employeeRole")?.toLowerCase() === "admin";
...
<TextField
  ...
  InputLabelProps={{ shrink: true }}
  inputProps={{
    min: task.taskAssignedDate
      ? new Date(new Date(task.taskAssignedDate).getTime() + 2 * 86400000)
          .toISOString()
          .split("T")[0]
      : today,
  }}
...
>
  ...
  <Select
    ...
    onChange={(e) => setTask({ ...task, assignedEmployees: e.target.value }) }
    renderValue={(selected) => selected.join(", ")}
  >
    {employees.map((employee, index) => (
      <MenuItem key={employee.id || index} value={employee.employeeEmail}>
        <Checkbox
          checked={task.assignedEmployees?.includes(employee.employeeEmail) || false}
        />
